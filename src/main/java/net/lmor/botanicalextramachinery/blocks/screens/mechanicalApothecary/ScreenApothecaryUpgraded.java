...
public class ScreenApothecaryUpgraded extends ExtraScreenBase<ContainerApothecaryUpgraded> {
    ...
    protected void renderBg(@Nonnull PoseStack poseStack, float partialTicks, int mouseX, int mouseY) {
        this.drawDefaultGuiBackgroundLayer(poseStack, LibResources.UPGRADED_MECHANICAL_APOTHECARY_GUI, screenAddInventory,
                new int[] {blockEntity.getFluidInventory().getFluidAmount()}, new int[] {blockEntity.getFluidInventory().getCapacity()}, bars, slotInfo);

        ScreenDrawLabelText.drawLabelText(poseStack, this.font, "block.botanicalextramachinery.upgraded_apothecary",
        ...
        if (this.blockEntity.getInventory().getStackInSlot(0).isEmpty()) {
            GhostItemRenderer.renderGhostItem(items, poseStack, this.leftPos + 90, this.topPos + 95);
        }
        ...
        if (this.blockEntity.getProgress() > 0) {
            ...
            RenderHelper.drawTexturedModalRect(poseStack, this.leftPos + 93, this.topPos + 41, this.imageWidth, 0, Math.round(11.0F * pctProgress), 37);
        }
