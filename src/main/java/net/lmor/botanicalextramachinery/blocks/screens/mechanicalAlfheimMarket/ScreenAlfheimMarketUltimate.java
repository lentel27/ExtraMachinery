...
public class ScreenAlfheimMarketUltimate extends ExtraScreenBase<ContainerAlfheimMarketUltimate> {
    ...
    protected void renderBg(@Nonnull PoseStack poseStack, float partialTick, int mouseX, int mouseY) {
        this.drawDefaultGuiBackgroundLayer(poseStack, LibResources.ULTIMATE_ALFHEIM_MARKET_GUI, screenAddInventory,
                new int[] {blockEntity.getCurrentMana()}, new int[] {blockEntity.getMaxMana()}, bars, slotInfo);

        ScreenDrawLabelText.drawLabelText(poseStack, this.font, "block.botanicalextramachinery.ultimate_alfheim_market",
        ...
        if (blockEntity.getInventory().getStackInSlot(0).isEmpty() && this.minecraft != null){
            GhostItemRenderer.renderGhostItem(new ItemStack(ModItems.catalystManaInfinity), poseStack, this.leftPos + 90 , this.topPos + 96);
        }
        ...
        if (blockEntity.getProgress() > 0) {
            ...
            RenderHelper.drawTexturedModalRect(poseStack, this.leftPos + 90, this.topPos + 61, this.imageWidth, 0, Math.round(16.0F * pct), 16);
        }
