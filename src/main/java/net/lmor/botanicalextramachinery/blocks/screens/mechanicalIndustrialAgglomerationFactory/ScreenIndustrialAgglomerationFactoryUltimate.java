...
public class ScreenIndustrialAgglomerationFactoryUltimate extends ExtraScreenBase<ContainerIndustrialAgglomerationFactoryUltimate> {
    ...
    protected void renderBg(@Nonnull GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        this.drawDefaultGuiBackgroundLayer(guiGraphics, LibResources.ULTIMATE_INDUSTRIAL_AGGLOMERATION_FACTORY_GUI);
        ...
        for (int i = 0; i < 2; i++){
            if (blockEntity.getInventory().getStackInSlot(i).isEmpty() && this.minecraft != null){
                ...
                GhostItemRenderer.renderGhostItem(items, guiGraphics, this.leftPos + 9 + (149 * i), this.topPos + 84);
            }
        }
        ...
        if (blockEntity.getProgress() > 0) {
            ...
            RenderHelper.drawTexturedModalRect(guiGraphics, LibResources.ULTIMATE_INDUSTRIAL_AGGLOMERATION_FACTORY_GUI, this.leftPos + 72, this.topPos + 58, this.imageWidth, 0, 40, height);
        }
    }
