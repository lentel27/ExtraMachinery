...
import net.lmor.botanicalextramachinery.config.LibXClientConfig.RenderingVisualContent;
    ...
    protected void doRender(@Nonnull BlockEntityApothecaryUpgraded tile, float partialTick, @Nonnull PoseStack poseStack, @Nonnull MultiBufferSource buffer, int light, int overlay) {
        if (RenderingVisualContent.all && RenderingVisualContent.ApothecarySettings.apothecaryUpgraded) {
            ...
            if (tile.getFluidInventory().getFluidAmount() > 0) {
                ...
                TextureAtlasSprite sprite = (TextureAtlasSprite)Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(fluidStack.getFluid().getAttributes().getStillTexture(fluidStack));
